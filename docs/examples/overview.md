...
sidebar_position: 1
title: "Overview"
description: "Complete, runnable Nebra programs, from a first script to a three-package project that spans Nebra and plain Lua."
---

# Examples

Every example on these pages is a real program that compiles and runs. The sources live in the
[`examples/` directory](https://github.com/nebra-lang/nebra/tree/master/examples) of the Nebra repository,
...
| [Typing existing Lua](./typing-existing-lua.md) | Wrapping a plain `.lua` library in a `.d.neb` so callers get types without a rewrite |
| [Compile-time annotations](./annotations.md) | Writing an annotation that rewrites the IR before codegen |

## Reading order

If you are new to Nebra, take them in the order above. Each one builds on the previous, and the last
...
**`examples/nebra-strings`** is a small library written entirely in Nebra. It exports string helpers
from `init.neb` and has a sub-module in `case.neb`, so it demonstrates how a package exposes both a
...
`init.d.neb` that describes its types. Nothing about it was rewritten for Nebra.
