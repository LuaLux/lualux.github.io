...
slug: /
title: "Introduction"
description: "What Lux is, what it compiles to, and how to find your way around the documentation."
---

# Introduction

Lux is a typed superset of Lua. You write `.lux` files, the compiler checks them and emits plain
Lua that any Lua 5.1, 5.2, 5.3, 5.4 or LuaJIT runtime can load. Nothing about Lux exists at run
...
## The two rules Lux is built on

**Every valid Lua program is a valid Lux program.** You can rename a `.lua` file to `.lux` and it
...
If you are already writing Lux and want a specific answer, the
[language reference](./language/types.md) covers each feature in its own page, and the
[lux.toml reference](./advanced/configuration.md) documents every configuration key.

## A note on reading the docs

Code blocks marked `lux` are Lux source. Code blocks marked `lua` are compiler output or plain Lua
being described. Where a page shows both, the Lux block comes first and the generated Lua follows,
